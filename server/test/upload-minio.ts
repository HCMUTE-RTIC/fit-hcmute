...
async function runTest() {
  ...
  try {
    ...
    console.log(
      `✅  Đăng nhập thành công! Token: ${token.substring(0, 30)}...\n`,
    );
    ...
    if (!uploadReq.ok) {
      throw new Error(
        `Upload ảnh lẻ thất bại! HTTP ${uploadReq.status} - ${await uploadReq.text()}`,
      );
    }
    ...
    if (!batchReq.ok) {
      throw new Error(
        `Upload chùm ảnh thất bại! HTTP ${batchReq.status} - ${await batchReq.text()}`,
      );
    }
    ...
    console.log(
      `Đã đẩy lên CSDL và S3 minio ${batchRes.data.length} file tĩnh. URLs public:`,
    );
