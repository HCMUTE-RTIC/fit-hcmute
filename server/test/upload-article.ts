...
async function runTest() {
    ...
    try {
        ...
        if (!loginReq.ok) throw new Error(`Login thất bại! HTTP ${loginReq.status}`);
        ...
        let getRes1 = await fetch('http://localhost:3000/articles');
        let data1 = await getRes1.json();
        ...
        // -----------------------------------------------------------------
        console.log('3️⃣  Lấy toàn bộ bài viết lần 2 (Mong đợi lấy từ Redis Cache)...');
        console.time('Fetch-List-2-Cache');
        let getRes2 = await fetch('http://localhost:3000/articles');
        await getRes2.json();
        ...
        console.log(`✅ Đã tạo bài 2 trùng tên! URL Slug đã bị thay đổi để không trùng lặp: ${article2.slug}`);
        ...
        let getRes3 = await fetch('http://localhost:3000/articles');
        let data3 = await getRes3.json();
        const finalCount = data3.data ? data3.data.length : data3.length;
        console.log(`   -> Có ${finalCount} bài viết. (Mong đợi là [${startCount} + 2] = ${startCount + 2})`);
