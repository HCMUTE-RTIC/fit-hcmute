...
async function runTest() {
  ...
  try {
    ...
    if (!loginReq.ok)
      throw new Error(
        `Login thất bại! HTTP ${loginReq.status} - ${await loginReq.text()}`,
      );
    ...
    if (!uploadReq.ok)
      throw new Error(`Upload thất bại! HTTP ${uploadReq.status}`);
    ...
    if (!createReq.ok)
      throw new Error(`Tạo Album thất bại: ${await createReq.text()}`);
    ...
    console.log(
      `   -> Auto Cover Photo: ${newAlbum.coverPhotoId === mediaIds[0] ? 'ĐÚNG (Lấy ảnh đầu tiên)' : 'SAI'}`,
    );
    ...
    if (!getReq.ok)
      throw new Error(`Lấy chi tiết Album thất bại: ${await getReq.text()}`);
    const fetchedAlbum = await getReq.json();
    console.log(
      `✅  Fetch thành công! Album có chứa ${fetchedAlbum.medias?.length || 0} tấm ảnh đính kèm.`,
    );
    ...
    if (!updateReq.ok)
      throw new Error(`Cập nhật Album thất bại: ${await updateReq.text()}`);
